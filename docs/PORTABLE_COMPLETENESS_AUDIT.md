...
| Pointer/touch | Reachable | X11, Wayland, and Win32 share motion/button/wheel targeting; Wayland binds `wl_touch`, while Windows/X11 retain their native pointer-emulation paths. Linux backends install a default pointer cursor (X11 scaled core cursor; Wayland `wp_cursor_shape_v1` or a scaled shm arrow) and snap `axis_value120` wheels to the same ticks (`axis_stop` clears the discrete latch). X11 maps XI2 touch to the shared pointer contract when the device is not already pointer-emulating. |
| Clipboard/IME/accessibility | Reachable with adapter boundary | Windows Unicode clipboard/IME, X11 ICCCM `CLIPBOARD`/`PRIMARY` (INCR, UTF8_STRING then STRING/TEXT/GTK text MIME including `text/plain;charset=utf8`, `text/uri-list`, receive-only `text/x-uri-list` / `text/rtf`, receive-only desktop file-list MIME (GNOME/Nautilus/KDE/Mozilla/KDE suggested-filename), and `UTF16_STRING` / receive-only `text/plain;charset=utf-16` / `utf16`, `TIMESTAMP`, `MULTIPLE` atom-pair requests, receive-only `text/html` / `text/rtf`, clipboard-manager handoff, UTF-8 BOM strip / UTF-16 decode / CR/LF→LF; paste prefers the owner's TARGETS list and a user ConvertSelection timestamp; XDND prefers TARGETS and the drop timestamp and emits Position hover via TranslateCoordinates; LeaveNotify / `wl_pointer.leave` emit `.up` for a held button then `(-1,-1)`; X11 EnterNotify with Button1/Button3 already down emits `.down` then queues the hover (no middle-down); implicit-grab MotionNotify after LeaveNotify does not restore hover and a later ButtonRelease for an already-released button is dropped; XdndLeave clears hover; incoming Wayland DnD motion updates hover; middle-click pastes PRIMARY as typed keys, with `xclip`/`xsel` PRIMARY fallback; CLIPBOARD paste does not read PRIMARY and uses local text only while this client owns CLIPBOARD; PRIMARY paste uses local text only while this client owns PRIMARY; ConvertSelection / INCR stashes up to 256 intervening events; Shift+Insert / XF86Paste paste CLIPBOARD as typed keys; receive-only `COMPOUND_TEXT` decodes to UTF-8 including ISO-8859-2 `ESC - B`, ISO-8859-3 `ESC - C`, ISO-8859-4 `ESC - D`, ISO-8859-15 `ESC - b`, ISO-8859-9 `ESC - M`, ISO-8859-5 `ESC - L`, ISO-8859-7 `ESC - F`, ISO-8859-6 `ESC - G`, and ISO-8859-8 `ESC - H`; invalid UTF-8 clipboard and drop bytes decode as Latin-1; receive-only ISO-8859-1/2/3/4/5/6/7/8/9/13/15, Windows-1250/1251/1252/1253/1254/1255/1256/1257, and KOI8-R charset MIME (both common casings on X11, plus `latin1`/`latin9`/`latin5`/`latin2`/`cyrillic`/`greek` aliases and the bare `KOI8-R` target) and Markdown decode to UTF-8; X11 extra mouse buttons 6–9 and wheel releases are ignored; X11 Enter/Leave ignore grab/ungrab and FocusIn/Out ignore grab/ungrab plus pointer-focus details; extra KDE5 / Mozilla-priv / KDE suggested-filename file MIME yields a local path), and Wayland `wl_data_device` plus `zwp_primary_selection_v1` are live, with `xclip`/`xsel`/`wl-copy`/`wl-paste --primary` as fallback. Text and `file:` drops use XDND / `wl_data_device` and are injected as typed keys (`file://` localhost / `127.0.0.1` / `[::1]` / local nodename are local paths); Wayland sends `data_offer.set_actions(copy)` on accept and `data_offer.finish` after every drop when the data device is v3+ (including failed or empty offers) and accepts UTF-16, `text/html`, `text/rtf`, `text/x-uri-list`, and desktop file-list MIME on receive only. Middle-click pastes PRIMARY as typed keys. Keyboard enter refreshes text-input. Wayland accepts text-input-v3 commits with a multiline hint and composer-strip cursor rectangle, and de-dupes only the confirming keysym. X11 claims focus via `WM_TAKE_FOCUS`/FocusIn, resets compose on FocusOut, honors group bits 13–14, MappingNotify, and GetModifierMapping, advertises `WM_LOCALE_NAME` plus `_NET_WM_ALLOWED_ACTIONS`, sets `_NET_WM_ICON` at 16/32/64/128 and an ICCCM `WM_HINTS` icon pixmap/mask at 32@1 / 64@2 (reinstalled on scale change) and `_NET_STARTUP_ID`, sends the startup-notification remove after map, exports a unique startup id for `xdg-open`, restores the cursor on EnterNotify, and raises urgency on `notify` until FocusIn or `_NET_WM_STATE_FOCUSED` (`notify-send --urgency=normal --icon=applications-internet`). Wayland uses keymap groups 2–4 with wrap, restores held modifiers from the keyboard-enter keys array, applies Caps Lock from the conventional Lock modifier bit, consumes `XDG_ACTIVATION_TOKEN` via `xdg_activation_v1` for startup focus and requests a fresh token for `xdg-open`, requests server-side decorations when advertised (retrying SSD once on a CSD configure), coalesces `present()` behind `wl_surface.frame`, and sets `xdg_toplevel_icon_v1` (32@1 plus 64@2, reinstalled on integer scale change) when advertised. Text-input disables when the toplevel is not activated. Named Armenian, Georgian, Thai, extra Ukrainian/Belarusian/Serbian/Macedonian Cyrillic, Latin-3, and Latin-4 keysyms type without an IME. Middle-click pastes PRIMARY; Shift+Insert / XF86Paste paste CLIPBOARD as typed keys. Both Linux backends track maximized/fullscreen window state; X11 also tracks hidden/shaded plus ICCCM `WM_STATE` / `WM_CHANGE_STATE`, skips Expose while fully obscured, and skips `present()` while NET hidden, ICCCM iconic, unmapped, shaded, or fully obscured (MapWindow reads the initial `_NET_WM_STATE` / `WM_STATE`; becoming hidden/unmapped/shaded clears hover so MapNotify / leaving-hidden QueryPointer can seed it; `_NET_WM_STATE` without HIDDEN cannot clear ICCCM Iconic), and Wayland records tiled/suspended plus `wm_capabilities` / `configure_bounds` when xdg-shell is v5+, skips `present()` while suspended, and exposes when leaving suspended or gaining activated. X11 FocusIn, leaving hidden, and gaining `_NET_WM_STATE_FOCUSED` expose. X11 publishes `_NET_WM_USER_TIME` on a dedicated `_NET_WM_USER_TIME_WINDOW` as well as the toplevel (DestroyNotify of that child recreates it; only the toplevel DestroyNotify closes). NumLock XOR Shift selects keypad digits on both backends. X11 Mod3 Mode_switch selects group 2. A Wayland copy before the first seat serial is advertised once a serial arrives. A full IME editor and AT-SPI/UIA provider transport remain out of scope. |
| DPI scaling | Reachable | Wayland tracks entered `wl_output` integer scale, `wl_surface.preferred_buffer_scale` when the compositor is bound at v6, output millimeter geometry when no scale event arrives, and `wp_fractional_scale_v1`/`wp_viewporter` when advertised, and refreshes the fallback shm cursor and toplevel icon when integer or fractional scale changes; X11 uses `GDK_SCALE`×`GDK_DPI_SCALE` / `QT_SCALE_FACTOR` / `QT_SCREEN_SCALE_FACTORS` / `Xft.dpi`, re-reads `Xft.dpi` on root `RESOURCE_MANAGER` changes, reads XSETTINGS `Gdk/WindowScalingFactor` / `Xft/DPI` and re-watches the XSETTINGS owner after DestroyNotify or a scale refresh, maps XI2 touch to pointer events when the device is not pointer-emulating, listens for RANDR `ScreenChangeNotify`, caches per-output millimeters so a window move can refresh integer scale, falls back to screen millimeter size, and reinstalls the scaled cursor plus physical WM size hints; both Linux backends size the default pointer from `XCURSOR_SIZE` / `Xcursor.size` / framebuffer scale; Win32 uses per-monitor-v2 logical geometry and scaled presentation. |
