...
| Clipboard/IME/accessibility | Reachable with adapter boundary | Windows Unicode clipboard/IME, X11 ICCCM `CLIPBOARD`/`PRIMARY` (INCR, UTF8_STRING then STRING/TEXT/GTK text MIME including `text/plain;charset=utf8` and `text/uri-list`, `TIMESTAMP`, clipboard-manager handoff), and Wayland `wl_data_device` plus `zwp_primary_selection_v1` are live, with `xclip`/`xsel`/`wl-copy` as fallback. Text and `file:` drops use XDND / `wl_data_device` and are injected as typed keys. Wayland accepts text-input-v3 commits with a multiline hint and composer-strip cursor rectangle, and de-dupes only the confirming keysym. X11 claims focus via `WM_TAKE_FOCUS`/FocusIn, advertises `WM_LOCALE_NAME` plus `_NET_WM_ALLOWED_ACTIONS`, sets `_NET_WM_ICON`, and raises urgency on `notify`. Wayland sets `xdg_toplevel_icon_v1` when advertised. Both Linux backends track maximized/fullscreen window state. A full IME editor and AT-SPI/UIA provider transport remain out of scope. |
| DPI scaling | Reachable | Wayland tracks entered `wl_output` integer scale and `wp_fractional_scale_v1`/`wp_viewporter` when advertised; X11 uses `GDK_SCALE`×`GDK_DPI_SCALE` / `QT_SCALE_FACTOR` / `QT_SCREEN_SCALE_FACTORS` / `Xft.dpi` and re-reads `Xft.dpi` on root `RESOURCE_MANAGER` changes; both Linux backends size the default pointer from `XCURSOR_SIZE` / `Xcursor.size` / framebuffer scale; Win32 uses per-monitor-v2 logical geometry and scaled presentation. |
